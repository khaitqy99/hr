...
const Layout: React.FC<LayoutProps> = ({ children, user, currentView, setView, onLogout }) => {
  ...
  // HR và nhân viên có các tab chức năng nhân viên giống nhau
  let views: string[];
  if (user.role === UserRole.ADMIN) {
    views = ['admin', 'salary-management'];
  } else {
    // HR và nhân viên đều có các tab giống nhau (không có tab nghỉ phép)
    views = ['dashboard', 'checkin', 'shifts', 'payroll'];
  }
  ...
  if (user.role === UserRole.ADMIN) {
    ...
    // Nhân viên và HR có các tab chức năng nhân viên (không có tab nghỉ phép - chỉ admin quản lý)
    navConfig.push(
