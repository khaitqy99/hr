...
import { getAttendance, getShiftRegistrations, getNotifications, getAnnualLeaveSummary, getBranches } from '../services/db';
...
const Dashboard: React.FC<DashboardProps> = ({ user, setView }) => {
  ...
  const loadData = useCallback(async () => {
    try {
      ...
      const [attendanceData, shiftsData, notifications, branchesData, leaveSummary] = await Promise.all([
        requests[0].catch(() => []),
        requests[1].catch(() => []),
        requests[2].catch(() => []),
        requests[3].catch(() => []),
      ...
      const branch = user.branchId
        ? (branchesData || []).find(b => b.id === user.branchId && b.isActive)
        : undefined;
      if (branch) {
        const shortAddress = branch.address?.split(',')[0]?.trim();
        setBranchLabel(shortAddress || branch.name);
      } else {
        setBranchLabel(null);
      }
  ...
  }, [user.id, user.contractType, user.branchId]);
  ...
  const todayHours = useMemo(() => {
    ...
    return parseFloat(dayWorkedHoursGrossMinusLunch(todayRecs).toFixed(1));
  }, [attendance]);
  ...
  if (isLoading && attendance.length === 0) {
    return (
      <div className="space-y-6">
        ...
        <SkeletonLoader variant="card" height={120} className="w-full" />
      </div>
    );
  }
  ...
  return (
    <PullToRefresh onRefresh={loadData}>
      ...
      <div className="fade-up" style={{animationDelay: '0ms'}}>
        <div className="relative overflow-hidden rounded-3xl bg-gradient-to-r from-blue-600 to-cyan-500 p-6 text-white shadow-lg shadow-blue-200">
          ...
          <div className="absolute top-4 right-4 z-20 flex items-center gap-2">
            {branchLabel && (
              <span className="text-[10px] font-bold max-w-[5.5rem] truncate" title={branchLabel}>
                {branchLabel}
              </span>
            )}
            ...
            <div className="flex flex-col gap-2 mt-2">
              <div className="flex flex-wrap gap-2">
                ...
                <div className="flex-1 min-w-[5.5rem] bg-black/10 rounded-2xl p-3">
                  ...
                  <p className="text-base sm:text-lg font-bold">{todayHours}h</p>
                </div>
                {todayOTHours > 0 && (
      ...
      {/* Phép năm + OT — một card gọn */}
      <div
        className="bg-white rounded-2xl shadow-sm border border-sky-50 px-3 py-2.5 fade-up flex items-stretch divide-x divide-slate-100"
          ...
          <div className="flex-1 min-w-0 pr-3 flex items-center justify-between gap-2">
            <div className="min-w-0">
              <p className="text-[10px] font-semibold text-slate-400 uppercase tracking-wide">
                Phép năm {annualLeave.year}
              </p>
              <p className="text-xs text-slate-500 mt-0.5 truncate">
                Đã dùng {annualLeave.usedDays}
                {annualLeave.pendingDays > 0 ? ` · chờ ${annualLeave.pendingDays}` : ''}
              </p>
            </div>
            <p className="text-lg font-bold text-emerald-600 shrink-0 tabular-nums">
              {annualLeave.remainingDays}
              <span className="text-xs font-medium text-slate-400">/{annualLeave.entitlementDays}</span>
            </p>
          </div>
        )}
        <div className={`flex-1 min-w-0 flex items-center justify-between gap-2 ${annualLeave && user.contractType !== ContractType.TRIAL ? 'pl-3' : ''}`}>
          <p className="text-[10px] font-semibold text-slate-400 uppercase tracking-wide shrink-0">
            OT tuần
          </p>
          <p className="text-lg font-bold text-orange-600 tabular-nums">{weekOTHours}h</p>
        </div>
      </div>
